/**
 * Análisis estadístico sobre registros de sacrificio vacuno.
 * Usa totales `total1_*` / `total2_*` en columnas Cbz_m / Kg_m; si vienen en cero, suma categorías de detalle.
 */

const CATEGORIAS_HEMBRA = ['terneras', 'aniojas', 'novillas', 'vacas'];
const CATEGORIAS_MACHO = ['terneros', 'aniojos', 'novillos', 'bueyes'];
...
export function metricasDia(reg) {
  let kgH = num(reg.total1_Kg_m);
  let cabH = num(reg.total1_Cbz_m);
  let kgM = num(reg.total2_Kg_m);
  let cabM = num(reg.total2_Cbz_m);

  if (kgH + kgM + cabH + cabM === 0) {
    for (const c of CATEGORIAS_HEMBRA) {
      kgH += num(reg[`${c}_Kg_m`]);
      cabH += num(reg[`${c}_Cbz_m`]);
    }
    for (const c of CATEGORIAS_MACHO) {
      kgM += num(reg[`${c}_Kg_m`]);
      cabM += num(reg[`${c}_Cbz_m`]);
    }
  }
...
export const TODAS_CATEGORIAS = [...CATEGORIAS_HEMBRA, ...CATEGORIAS_MACHO];
