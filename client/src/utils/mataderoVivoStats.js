 ...
 * Matadero vivo: totales `total1_*` / `total2_*` en columnas Cab / Kg (par final); si vienen en cero, suma categorías de detalle.
 */
...
export function metricasDia(reg) {
  let kgH = num(reg.total1_Kg);
  let cabH = num(reg.total1_Cab);
  let kgM = num(reg.total2_Kg);
  let cabM = num(reg.total2_Cab);
