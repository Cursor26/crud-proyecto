...
const MataderoVivo = () => {
    ...
    const getRegistros = () => {
        Axios.get('http://localhost:3001/matadero')
            .then(res => {
    ...
    const handleSubmit = (e) => {
        ...
        if (editando) {
            Axios.put(`http://localhost:3001/update-matadero/${fechaOriginal}`, data)
                .then(() => {
            ...
            Axios.post('http://localhost:3001/create-matadero', data)
                .then(() => {
    ...
    const eliminarRegistro = (fecha) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Axios.delete(`http://localhost:3001/delete-matadero/${fechaSolo}`)
...
.then(() => {
    ...
    const exportarExcel = () => {
        ...
        exportRowsToExcel(rows, 'Matadero', `matadero_vivo_${new Date().toISOString().slice(0, 10)}.xlsx`);
    };
...
<h4>Gestión de Matadero Vivo</h4>
                    ...
                    <small className="text-muted">
                        Administración del flujo de la producción en el Matadero (RF20 Excel, RF22 responsables)
                    </small>
