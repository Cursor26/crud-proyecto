...
import MataderoVivoStatsPanel from './MataderoVivoStatsPanel';
...
const MataderoVivo = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editando) {
      Axios.put(`/update-matadero/${fechaOriginal}`, data)
        .then(() => {
      ...
      Axios.post('/create-matadero', data)
        .then(() => {
  ...
  const eliminarRegistro = (rawFecha) => {
    ...
    Swal.fire({
      title: 'Confirmar eliminación — Matadero vivo',
      icon: 'warning',
      showCancelButton: true,
      ...
      html: (
        ...
        + `${fechaSolo}</strong> (Matadero vivo).</p>`
      ...
      if (result.isConfirmed) {
        Axios.delete(`/delete-matadero/${fechaSolo}`)
          .then(() => {
  ...
  const mataderoExportAepg = useMemo(() => {
  ...
  return (
    <div className="container-fluid px-0">
      <ModuleTitleBar
        title="Gestión de Matadero Vivo"
        actions={
          <>
            ...
            <ExportacionAepgGrupo
              ...
              subtitulo="Reporte: matadero en vivo. Generado con la gestión de producción/estadística AEPG."
              descripcion="Renglones con categorías, kg, cabezas y trazabilidad (creado/actualizado por)."
              nombreBaseArchivo={`AEPG_matadero_vivo_${new Date().toISOString().slice(0, 10)}`}
              sheetName="Matadero"
              headers={mataderoExportAepg.headers}
              dataRows={mataderoExportAepg.dataRows}
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Estadísticas, análisis e informes — Matadero vivo</Modal.Title>
        </Modal.Header>
        <Modal.Body className="p-0" style={{ maxHeight: 'min(90vh, 900px)' }}>
          <div className="p-2 p-md-3 overflow-auto" style={{ maxHeight: 'min(90vh, 900px)' }}>
            <MataderoVivoStatsPanel registros={registros} />
          </div>
        </Modal.Body>
      </Modal>
        ...
        <form onSubmit={handleSubmit} noValidate>
          <Modal.Body className="modal-form-body-scroll">
            <ProduccionFormInstrucciones
              titulo="Cómo completar un día de registro (Matadero vivo)"
              nombreModulo="Matadero vivo"
            />
            <div className="row mb-3 align-items-end g-2">
              <div className="col-md-4 col-lg-3">
                <label className="form-label" htmlFor="produccion-fecha-matadero">Fecha del registro</label>
                <input
                  id="produccion-fecha-matadero"
                  type="date"
                  ...
                  disabled={editando}
                />
                <p className="form-text small mb-0" id="produccion-fecha-help-matadero">
