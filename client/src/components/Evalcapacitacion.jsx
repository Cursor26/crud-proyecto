...
const Evalcapacitacion = () => {
    ...
    const guardar = () => {
        ...
        if (editando) {
            Axios.put(`/update-evalcapacitacion/${idOriginal}`, data)
                .then(() => {
            ...
            Axios.post('/create-evalcapacitacion', data)
                .then(() => {
    ...
    const eliminarRegistro = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                Axios.delete(`/delete-evalcapacitacion/${id}`)
                    .then(() => {
    ...
    const evalCapExportAepg = useMemo(() => {
    ...
    return (
        <div className="container-fluid px-0">
            <ModuleTitleBar
                title="Gestión de Evaluación de Capacitación"
                actions={
                    <>
                        <ExportacionAepgGrupo
                            tituloSistema={AEPG_TITULO_RRHH}
                            subtitulo="Reporte: evaluación de capacitación."
                            descripcion="Listado filtrado: empleado, estado y descripción (sin acciones)."
                            nombreBaseArchivo={`AEPG_evalcapacitacion_${new Date().toISOString().slice(0, 10)}`}
                            sheetName="EvalCapacitacion"
                            headers={evalCapExportAepg.headers}
                            dataRows={evalCapExportAepg.dataRows}
            ...
            <FormModal
                ...
                title={editando ? 'Editar eval. capacitación' : 'Nueva eval. capacitación'}
                onPrimary={guardar}
