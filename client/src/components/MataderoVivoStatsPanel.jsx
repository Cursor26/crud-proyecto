...
import {
  ...
  serieConMediaMovil,
  topDias,
} from '../utils/mataderoVivoStats';
import MataderoVivoInteractivo from './MataderoVivoInteractivo';
...
function exportarMultihoja(
  ...
  const h1 = [
    ...
    { concepto: 'Suma Kg (Cab / Kg — totales y detalle)', valor: res.sumKg },
 ...
 */
function MataderoVivoStatsPanel({ registros }) {
  ...
  const handleExportInforme = () => {
    ...
    const nombre = `informe_stats_matadero_${new Date().toISOString().slice(0, 10)}.xlsx`;
    exportarMultihoja(
      {
        res,
        comp,
  ...
  return (
    ...
    <Card className="mb-3 border-0 shadow-sm">
      <Card.Header as="h2" className="h5 mb-0 py-3 d-flex flex-wrap align-items-center gap-2">
        ...
        <span className="text-muted small ms-auto" style={{ fontWeight: 400 }}>
          (totales y detalle: columnas Cab / Kg, total1 / total2)
        </span>
      </Card.Header>
    ...
    <MataderoVivoInteractivo registros={registros} />
    </>
  );
}

export default MataderoVivoStatsPanel;
