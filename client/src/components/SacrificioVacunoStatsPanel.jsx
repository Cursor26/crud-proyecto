...
import SacrificioVacunoInteractivo from './SacrificioVacunoInteractivo';
...
function exportarMultihoja(
  ...
  const h1 = [
    ...
    { concepto: 'Suma Kg (col. Cbz_m / Kg_m — totales y detalle)', valor: res.sumKg },
  ...
  const distHoja = dist.aplica
    ? [
        { parte: 'Hembra (total1)', kg: dist.kgH, porciento: dist.hembrasPct },
        { parte: 'Macho (total2)', kg: dist.kgM, porciento: dist.machosPct },
      ]
    : [{ nota: 'Sin kg para desglosar' }];
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(distHoja), 'Hembra Macho');
 ...
 */
function SacrificioVacunoStatsPanel({ registros }) {
  ...
  const handleExportInforme = () => {
    ...
    const nombre = `informe_stats_sacrificio_${new Date().toISOString().slice(0, 10)}.xlsx`;
    exportarMultihoja(
      {
        res,
        comp,
  ...
  return (
    ...
    <Card className="mb-3 border-0 shadow-sm">
      <Card.Header as="h2" className="h5 mb-0 py-3 d-flex flex-wrap align-items-center gap-2">
        ...
        <span className="text-muted small ms-auto" style={{ fontWeight: 400 }}>
          (basado en columnas Cbz_m / Kg_m y totales total1 / total2)
        </span>
      </Card.Header>
      <Card.Body className="pt-0">
        ...
        <Accordion defaultActiveKey="0" flush>
          ...
          <Accordion.Item eventKey="6">
            <Accordion.Header>7) Distribución hembra (total1) vs macho (total2) — Kg (período)</Accordion.Header>
            <Accordion.Body>
              ...
              {dist.aplica && (
                <div>
                  <div className="d-flex align-items-center gap-2 mb-2 small">
                    <span>Hembra: {formateaNum(dist.kgH, 0)} kg ({formateaNum(dist.hembrasPct, 1)}%)</span>
                  </div>
                  <div
                  ...
                  <p className="mt-1 mb-0 small">Macho: {formateaNum(dist.kgM, 0)} kg ({formateaNum(dist.machosPct, 1)}%)</p>
                </div>
              )}
            </Accordion.Body>
          </Accordion.Item>
    ...
    <SacrificioVacunoInteractivo registros={registros} />
    </>
  );
}

export default SacrificioVacunoStatsPanel;
