...
export function normalizeVisibleColumns(stored, definitions) {
  ...
  const list = Array.isArray(stored) ? stored.filter((id) => allowed.has(id)) : defaultVisibleColumnIds(definitions);
  locked.forEach((id) => {
    if (!list.includes(id)) list.push(id);
  });
  return list.length ? list : defaultVisibleColumnIds(definitions);
}
